...
exports.viewStudent = async (req, res) => {
    ...
    res.render('student/viewStudent', { listStudent: listStudent, loginName: req.session.email })
}
            ...
            if (error) {
                fs.mkdir(path, (error) => {
                    if (error) {
                        ...
                        `;
                        content += '<h4 style="color: #0085ff"> From: ' + aStudent.email.toString() + '</h4> <hr>';
                        content += '<span style="color: black"> Idea name: ' + ideaName.toString() + '</span><br>';
                        content += '<span style="color: black"> Event name: ' + aEvent.name.toString() + '</span><br>';
                        if (!checkAnnonymously) {
                            content += '<span style="color: black"> Student name: ' + aStudent.name.toString() + '</span>';
                        }
                        else {
                ...
                res.render('student/addFileToIdea', { idEvent: idEvent, path: path, loginName: req.session.email });
            } else {
...
exports.searchEvent = async (req, res) => {
    ...
    res.render('student/viewSubmittedIdeas', { listEvent: listEvent, loginName: req.session.email });
}
...
exports.viewLastestIdeas = async (req, res) => {
    ...
    res.render('student/viewLastestIdeas', { listIdeas: last_ideas });
}
...
exports.viewEventDetail = async (req, res) => {
    ...
    try {
        ...
        let listDislikes = await dislikes.find({ studentID: { $all: student._id } });;
        ...
        function callBack() {
            if (listIdeas.length === counter) {
                ...
                res.render('student/viewEventDetail', { idEvent: id, listFiles: listFiles, compare: compare, noPage: noPage, loginName: req.session.email })
            };
        };
...
exports.viewMostComments = async (req, res) => {
    if (req.session.email === undefined) {
        ...
        try {
            ...
            res.render('student/viewMostComments', { mostViewedIdeas: mostViewedIdeas, loginName: req.session.email });
        } catch (e) {
            console.error(e);
            res.render('student/viewMostComments', { mostViewedIdeas: mostViewedIdeas, loginName: req.session.email });
        }
...
exports.viewLatestComments = async (req, res) => {
    if (req.session.email === undefined) {
        ...
        try {
            ...
            res.render('student/viewLatestComments', { lastComments_detail: lastComments_detail, loginName: req.session.email });
        }
        catch (err) {
            console.log(err);
            res.render('student/viewLatestComments', { lastComments_detail: lastComments_detail, loginName: req.session.email });
...
exports.paginations = async (req, res) => {
    ...
    if (page) {
        ...
        let listEvent = await event.find({}).skip(skip).limit(pageSize).then(data => {
            res.json(data);
        })
            .catch((err) => {
                res.status(500).json('loi');
            });
        res.render('student/testPagination', { listEvent: listEvent, loginName: req.session.email })
    } else {
        let listEvent = await event.find({})
            ;
        res.render('student/testPagination', { listEvent: listEvent, loginName: req.session.email })
...
exports.doChangePassword = async (req, res) => {
    ...
    try {
        ...
        if (!flag) {
            res.render('student/changePassword', { errors: errors, loginName: req.session.email })
        }
        else {
